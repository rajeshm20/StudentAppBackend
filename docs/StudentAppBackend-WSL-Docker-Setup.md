# StudentAppBackend — WSL + Docker Setup Guide

Vapor-based Swift backend that exposes student authentication APIs over REST and additional student APIs over GraphQL. This guide covers running the project on **Windows 11 with WSL2**, entirely through Docker.
...
- Ports:
  ...
  - App: use `8081` on host if `8080` is occupied by Jenkins or another service
...
> The Dockerfile builds Swift inside the container (`FROM swift:6.0-noble`), so **Swift does not need to be installed in WSL**.
...
If `8080` is already in use (for example, by Jenkins), map another host port:
...
> GitHub no longer accepts account passwords for Git operations. Use SSH or a Personal Access Token.
...
### Publishing via CI Instead

The backend container is also published to GHCR automatically via `.github/workflows/docker-publish.yml` (or your combined `swift.yml` CI/CD workflow).
...
## Complete Packaged Setup (Backend + MySQL)

The published image contains only the backend application. To run the backend together with MySQL, use `docker-compose.package.yml`:
...
To run the backend behind Caddy with TLS termination, use `docker-compose.caddy.yml`:
...
- Keep the app container on HTTP in Docker or production environments.
