...
private func configureMiddleware(_ app: Application) throws {
    ...
    // Register SecurityHeadersMiddleware at the beginning of the pipeline (ahead of ErrorMiddleware)
    // so that all responses—including error responses (4xx, 5xx) generated by ErrorMiddleware—
    // consistently receive security headers and HSTS when over HTTPS.
    app.middleware.use(SecurityHeadersMiddleware(environment: app.environment), at: .beginning)
